...
Rendering hair fibers is a challenging task because of the extensive amount of hair fibers required to render a realistic human hair model. Also hair fibers are very thin, requiring a lot of rays to be traced to produce noise-free images. Marschner et al.~\cite{marschner} proposed a physically based single fiber scattering model to render realistic human hair fibers. 

When rendering light colored hair, multiple fiber scattering is essential for the hair color. Zinke et al.~\cite{zinke} extended the work of Marschner by splitting multiple scattering up into two components: global multiple scattering and local multiple scattering. Global multiple scattering approximated the multiple scattering contribution, reducing the rendering time considerably, while local multiple scattering resembles the Marschner model to keep the single fiber scattering characteristics.

In this thesis the importance sampling strategy proposed by d'Eon et al.~\cite{eon2013} is used. This strategy works particularly well for the Marschner model. In this thesis the same importance sampling strategy will be applied to the dual scattering method to see if this method can be made even more efficient. Results are evaluated by comparing the variance measurements between uniform sampling and importance sampling. It can be shown that the importance sampling does not significantly show better results compared to uniform sampling.
